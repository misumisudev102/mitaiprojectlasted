    ...
    return async function ({ event }) {
        ...
        try {
            if (!allThreadID.includes(threadID) && event.isGroup == !![]) {
                ...
                for (singleData of threadIn4.userInfo) {
                    ...
                    try {
                        ...
                        logger(global.getText('handleCreateDatabase', 'newUser', singleData.id), '『 DATA - USER 』'));
                    } catch(e) { console.log(e) };
                }
                logger(global.getText('handleCreateDatabase', 'newThread', threadID), '『 DATA - GROUP 』');
            }
            if (!allUserID.includes(senderID) || !userName.has(senderID)) {
                ...
                logger(global.getText('handleCreateDatabase', 'newUser', senderID), '『 DATA - USER 』');
            }
